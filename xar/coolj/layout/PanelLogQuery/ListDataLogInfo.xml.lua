...
function DC_CreateUIObjectFromData(self, strItemObjId, nItemIndex, tItemData, nOperationType)
	...
	local itemObj = objFactory:CreateUIObject(strItemObjId, "LogInfo.List")
	return itemObj
end
...
function SetText(self, item)
	self:GetControlObject("text.user"):SetText(item['user'])
	self:GetControlObject("text.s_time"):SetText(os.date("%x", item['s_time']+0))
	self:GetControlObject("text.info"):SetText(item['info'])
	self:GetControlObject("text.ip"):SetText(item['ip'])
end
...
function OnShowDetails(self)
	local owner = self:GetOwnerControl()
	owner:FireExtEvent("OnItemEvent", "OnShowDetails", owner:GetControlObject("text.info"):GetText())
end