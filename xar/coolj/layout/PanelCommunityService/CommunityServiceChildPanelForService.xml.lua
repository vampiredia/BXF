...
function BES_OnClick(self)
	if self:GetText() == "编辑" then
		...
		--XLMessageBox(json.encode(GlobalDataTable))
		Set_CommunityServiceInfo(self:GetOwnerControl())
	end	
end
...
function Set_CommunityServiceInfo(self)
	local httpclient = XLGetObject("Whome.HttpCore.Factory"):CreateInstance()
	httpclient:AttachResultListener(
		function(result) 
			local response = json.decode(result)
	...
	local param = "action=jiazheng_list&service="..httpclient:EscapeParam(json.encode(GlobalDataTable))
	local url = "/service/jiazheng"
