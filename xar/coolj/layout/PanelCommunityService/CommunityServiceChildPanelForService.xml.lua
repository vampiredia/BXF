...
function Get_CommunityServiceInfo(self)
	...
	HttpRequest("/api/life/business?action=get_root_list", "GET", "", request)
end
...
function LBS_OnItemEvent(self, eventName, eventType, UserData, ItemObj)
	if eventType == "OnSelect" then
		...
		HttpRequest("/api/life/business?action=get_info_list&id="..UserData['id'], "GET", "", request)
	elseif eventType == "OnDelDetails" then
		ret = MessageBox(nil, "删除服务项目", "确认删除服务项目 "..ItemObj:GetName())
		if ret == 1 then
			local request = function(ret, msg, result)
				if ret == 0 then
					self:GetOwnerControl():Get_CommunityServiceInfo()
				else
					AddNotify(self, msg, 5000)
				end
			end
			local param = "action=del_root&id="..UserData['id']
			HttpRequest("/api/life/business", "POST", param, request)
		end
	elseif eventType == "OnEditDetails" then
		ret = NewServiceBox(nil, "修改分类", 'edit_community_service_root', UserData)
		if ret == 1 then
			self:GetOwnerControl():Get_CommunityServiceInfo()
		end
...
function LBSS_OnInitControl(self)
	local objFactory = XLGetObject("Xunlei.UIEngine.ObjectFactory")
	local headerTable = {
		{HeaderItemId='title', ItemWidth=88, Text="商户简称", TextLeftOffset=7, TextHalign="left", SubItem=false, MaxSize=88, MiniSize=88, ShowSplitter=true, ShowSortIcon=false, IncludeNext=false, SortProperty=0},
		{HeaderItemId='content_msg', ItemWidth=202, Text="老板消息", TextLeftOffset=7, TextHalign="left", SubItem=false, MaxSize=202, MiniSize=202, ShowSplitter=true, ShowSortIcon=false, IncludeNext=false, SortProperty=0},
	...
	local datasource = objFactory:CreateUIObject(1, "SubService.Community.DataSource")
	datasource:InitControl()
	local dataconverter = objFactory:CreateUIObject(2, "SubService.Community.DataConverter")
	dataconverter:InitControl()
	self:SetDataSourceAndDataConverter(datasource, dataconverter)
	self:ReloadData()
end
...
function Btn_NewService(self)
	ret = NewServiceBox(nil, "新建分类", 'add_community_service_root', {})
	if ret == 1 then
		self:GetOwnerControl():Get_CommunityServiceInfo()
	end
end
...
function Get_SubServiceInfo(self, pid)	
	...
	local param = ""
	local url = "/api/life/business?action=get_info_list&id="..pid
