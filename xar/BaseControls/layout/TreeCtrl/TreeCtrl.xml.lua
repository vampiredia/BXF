...
function SetAdjust(self,bVscroll)
	...
	for i=1,attr.ItemVisibleCount do
...
function TreeCtrl_OnVScroll(self, name, type, newpos)
	local owner = self:GetOwnerControl()
	local attr = owner:GetAttribute()
	if type == 4 then
		attr.m_top = math.ceil((22-self:GetScrollPos()) / 22)
...
function UpdateVScroll(self)
	...
	vscroll:SetScrollRange(0, totalVisible*22)
end
...
function MouseWheel(self, distance)
	...
	local step = 1
	if distance < 0 then
		if #mapTopItem >= attr.ItemVisibleCount then  
