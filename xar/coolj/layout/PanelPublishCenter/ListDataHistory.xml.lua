...
function DC_InitControl(self)
	local attr = self:GetAttribute()
	attr.ItemTotalWidth = 690
	attr.ItemTotalHeight = 34
end
...
function DC_CreateUIObjectFromData(self, strItemObjId, nItemIndex, tItemData, nOperationType)
	...
	local itemObj = objFactory:CreateUIObject(strItemObjId, "History.List")
	return itemObj
end
...
function SetText(self, item)
	...
	self:GetControlObject("text.nick"):SetText(item['nick'])
	self:GetControlObject("text.s_time"):SetText(os.date("%x", item['s_time']))
	if item['status'] == 'open' then 
		self:GetControlObject("text.status"):SetText("未答复")
	elseif item['status'] == 'answer' then 
		self:GetControlObject("text.status"):SetText("已答复")
		self:GetControlObject("text.status"):SetTextColorResID("system.green")
	elseif item['status'] == 'close' then 
		self:GetControlObject("text.status"):SetText("已删除")
		self:GetControlObject("text.status"):SetTextColorResID("system.red")
	end
end
...
function OnShowDetails(self)
	local owner = self:GetOwnerControl()
	owner:FireExtEvent("OnItemEvent", "OnShowDetails", owner:GetControlObject("text.id"):GetText())
end