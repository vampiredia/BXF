function GetRootClass(obj)
	if obj:GetClass() == "CoolJ.ScrollBar" then return obj end
	return GetRootClass(obj:GetOwnerControl())
end
...
function initBtnPosV(self)
	...
	bkgarea:SetObjPos(0,attr.NavigationBtnLength,w,b - attr.NavigationBtnLength)
end
...
function initBtnPosH(self)
	...
	local bkgarea = bkg:GetControlObject("bkgarea")
	bkgarea:SetObjPos(attr.NavigationBtnLength,0,r - attr.NavigationBtnLength,h)
end
...
function OnBind(self)
	...
	attr.ScrollAniTemplate = tm:GetTemplate("CoolJ.ScrollBar.ScrollPosAni.Template","AnimationTemplate")
	
	local bkg = self:GetControlObject("bkg")
	local bkgattr = bkg:GetAttribute()
	bkgattr.Type = attr.Type
	bkgattr.NormalBkgID = attr.BkgNormalBitmapID
    bkgattr.HoverBkgID = attr.BkgHoverBitmapID
	bkgattr.DownBkgID = attr.BkgDownBitmapID
	
	local prebtn = bkg:GetControlObject("prebtn") 
    local prebtnattr = prebtn:GetAttribute()
    prebtnattr.NormalBkgID = attr.PreBtnNormalBitmapID
    prebtnattr.HoverBkgID = attr.PreBtnHoverBitmapID
    prebtnattr.DownBkgID = attr.PreBtnDownBitmapID

	local nextbtn = bkg:GetControlObject("nextbtn")
	local nextbtnattr = nextbtn:GetAttribute()
	nextbtnattr.NormalBkgID = attr.NextBtnNormalBitmapID
    nextbtnattr.HoverBkgID = attr.NextBtnHoverBitmapID
    nextbtnattr.DownBkgID = attr.NextBtnDownBitmapID

    local thumbbtn = bkg:GetControlObject("thumbbtn")
    local thumbbtnattr = thumbbtn:GetAttribute()
	thumbbtnattr.NormalBkgID = attr.ThumbBtnNormalBitmapID
    thumbbtnattr.HoverBkgID = attr.ThumbBtnHoverBitmapID
    thumbbtnattr.DownBkgID = attr.ThumbBtnDownBitmapID
...
function ThumbBtn_OnMouseMove(btn,x,y,flag)
    ...
    local attr = sb:GetAttribute()
	if attr.Type == "h" then
        sb:FireExtEvent("OnHScroll", 4, 0)
    else
        sb:FireExtEvent("OnVScroll", 4, 0)
    end
end
